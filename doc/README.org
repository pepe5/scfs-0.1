...
*** design: cp -r  /tmp/dev/scwd  /tmp/dev/scwd2
    python lib/scatman del WD_UC2 ~/.scfs/cat1.sqlite
    python lib/scatman add /tmp/dev/scwd WD_UC2 ~/.scfs/cat1.sqlite
    echo 234 > /tmp/dev/scwd/b/234
    python lib/scatman add /tmp/dev/scwd/b WD_UC2 ~/.scfs/cat1.sqlite

*** setup
    :cleanup: rm -vr /tmp/dev/scwd/b/bb
    scatman d WD_2 ~/.scfs/cat1.sqlite
    
    :setup: scfs> scatman add /tmp/dev/scwd/ WD_2 ~/.scfs/cat1.sqlite
    scfs> sqlite3 ~/.scfs/cat1.sqlite 'select * from WD_2_files'
    ...
    :act: scfs> scatman add /tmp/dev/scwd/b/bb/ WD_2 ~/.scfs/cat1.sqlite
...
*** mount
   ...
   python lib/scatfs -d -s -o database=~/.scfs/cat1.sqlite ~/mnt/cat1
#(<) cdcatfs -s -o database=~/.scfs/cat1.sqlite ~/mnt/cat1
...
>>>>>>> da1458b3cf38ae223cd6ce81c100bfc7bfb7e888
   cd ~/text/scfs; python lib/scatman add /tmp/dev/scwd WD_UC2 ~/.scfs/cat1.sqlite
#*(<) cdcatman add doc/ CD_1 ~/.scfs/cat1.sqlite
   ls -l ~/mnt/cat1/CD_1
   cdcatman list ~/.scfs/cat1.sqlite
   sqlite3 ~/.scfs/cat1.sqlite .dump
...
* XATTR PART
** gvfs-info ~ info
   $> (echo .mode line; echo "select * from WD_UC2_files where fileName = 'c1';") | sqlite3 ~/.scfs/cat1.sqlite
...
** TODO fuse args
   server.parser.parse_args(['-o database=/home/user/.scfs/cat1.sqlite'])
...
** from pysqlite2 import dbapi2 as sqlite
    ...
    fileId = 6
    name = '123'
    con = sqlite.connect('/home/user/.scfs/cat1.sqlite')
