...
*** dev env depl: 
#  sudo install -v -d /usr/lib/python2.*/site-packages/Scfs/
    pushd /usr/lib/python2.*/site-packages/; sudo install -v Cdcatfs/*.py Scfs/
    sudo ln -sv -bf `pwd`/lib/CatalogCreator.py /usr/lib/python2.*/site-packages/Scfs/
...
** uc2 - insert
   cp -r  /tmp/dev/scwd  /tmp/dev/scwd2
   python lib/scatman del WD_UC2 ~/.scfs/cat1.db
   python lib/scatman add /tmp/dev/scwd2 WD_UC2 ~/.scfs/cat1.db
   echo 234 > /tmp/dev/scwd2/b/234
   python lib/scatman add /tmp/dev/scwd2/b WD_UC2 ~/.scfs/cat1.db
